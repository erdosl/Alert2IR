...
Alert2IR runtime composition remains the deterministic `MockBackend`. Supplying the API-config path at runtime, live backend construction, and Compose configuration remain deferred, so the implemented adapter is not yet exercised by the application request path. Generated credentials and other Velociraptor runtime state remain external and must not be committed.
