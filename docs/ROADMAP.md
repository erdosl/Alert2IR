...
12. **12 Observability** — active: add portable application telemetry and a failure-isolated open-source reference observability platform.
...
WS13 CrowdStrike remains planned but is deferred. A trial or evaluation has been requested, but no usable license or environment is available. No CrowdStrike discovery, API contract, capability mapping, or operational validation is claimed; implementation must wait for an available evaluation environment rather than being inferred from public documentation. These optional commercial deferrals do not block independent work: WS12 Observability is active, and the validated open-source path and routine CI remain the project baseline.

WS12 is active. Its accepted direction includes OpenTelemetry instrumentation, structured application logging, metrics, tracing, readiness semantics, and a separate open-source reference observability platform on `obs01` using Grafana, Prometheus, Alertmanager, Loki, Tempo, and Alloy, followed by dashboards and alerting. This Stage 1 decision and lab authorization do not implement or deploy those capabilities. The next WS12 stage is Git-tracked deployment configuration before any configuration is applied to `obs01`; application instrumentation, provisioning, dashboards, alerting, and operational validation remain incomplete.
