...
11. **11 Binalyze AIR** — deferred: discovery is complete; Stage 0 deployment verification is blocked pending requested trial/license/deployment access.
12. **12 Observability** — completed: portable application telemetry, a failure-isolated open-source reference platform, operator dashboards, deterministic alerting, and operational procedures.
13. **13 CrowdStrike** — deferred: requested trial/license access is unavailable; this optional integration is never required for core operation.
...
WS11 remains planned but is deferred. Discovery is complete, and its conditional candidate maps `process.list` to one AIR acquisition against one exact asset through a pre-provisioned Windows profile containing only `pri`; this is not yet an implementation contract. Stage 0 concluded `WS11 STAGE 0: BLOCKED` because no usable licensed or evaluation deployment, approved base URL or token source, or deployed organization, asset, profile, single-target request, task-lifecycle, or evidence-reference contract was available to verify. No AIR endpoint was contacted, and no task, acquisition, profile change, case, or endpoint operation was performed. Public API examples alone are insufficient for implementation. When access becomes available, resume at Stage 0 deployment/API-contract verification—not discovery or implementation—and verify the deployed version, TLS and authentication, one explicit organization, exact owned-asset mapping, an exact `pri`-only profile, a hard single-target safeguard, task and assignment lifecycle, evidence-reference semantics, and least-privilege authorization.

WS13 CrowdStrike remains planned but is deferred. A trial or evaluation has been requested, but no usable license or environment is available. No CrowdStrike discovery, API contract, capability mapping, or operational validation is claimed; implementation must wait for an available evaluation environment rather than being inferred from public documentation. These optional commercial deferrals do not block independent work: the validated open-source path, completed WS12 observability, and routine CI remain the project baseline.

WS12 is complete. Alert2IR now emits structured newline-delimited JSON logs and vendor-neutral OpenTelemetry metrics and traces, generates server request correlation, and exposes PostgreSQL/schema-only readiness. Native Alloy on `ir-core` forwards bounded host, container, application, probe, log, and trace telemetry through native Alloy on the separate `obs01` failure domain to Prometheus, Loki, and Tempo. Grafana has three source-provisioned dashboards; Prometheus has eight deterministic alert rules; Alertmanager routes only to `lab-null`; and live validation proved request-to-log-to-trace correlation, real alert firing/resolution, and continued application operation during a local Alloy outage. The canonical procedures and reference baselines are in [`docs/OBSERVABILITY.md`](OBSERVABILITY.md). The optional live `backend.operation.submitted` observability exercise remained deferred because no suitable already-online authorized endpoint was available; the event and ordering are implemented and deterministically tested, so this does not block WS12 closure.

With WS11 and WS13 still deferred pending external trial access, WS14 Packer is the next actionable roadmap workstream. No WS14 work begins as part of WS12 closure.
