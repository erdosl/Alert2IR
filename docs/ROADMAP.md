...
12. **12 Observability** — next actionable workstream: add operational signals justified by running-system needs.
...
WS13 CrowdStrike remains planned but is deferred. A trial or evaluation has been requested, but no usable license or environment is available. No CrowdStrike discovery, API contract, capability mapping, or operational validation is claimed; implementation must wait for an available evaluation environment rather than being inferred from public documentation. These optional commercial deferrals do not block independent work: WS12 Observability is the next actionable workstream, and the validated open-source path and routine CI remain the project baseline.
